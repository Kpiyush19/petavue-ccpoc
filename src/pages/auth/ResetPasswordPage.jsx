...
import { storage, logoutUser } from '../../components/google-auth';
...
export default function ResetPasswordPage() {
  ...
  useEffect(() => {
    if (tokenInvalid) {
      toast.error('Reset password token not valid');
      navigate('/login');
    }
  }, [tokenInvalid]);
  ...
  const handleSubmit = async (e) => {
    ...
    setResetting(true);
    try {
      await resetPassword.mutateAsync({ token, password: formData.password, confirmPassword: formData.confirmpassword });
      navigate('/login');
    } catch {
    } finally {
      setResetting(false);
      ...
      <div className="min-h-screen bg-gray-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
        <div style={{ boxShadow: '0px 8px 24px 0px rgba(105, 112, 149, 0.15)' }} className="mt-8 bg-white py-8 px-4 sm:rounded-xl sm:mx-auto sm:w-full sm:max-w-md">
          <div className="sm:mx-auto mb-4 sm:w-full sm:max-w-md">
            ...
            <h2 style={{ lineHeight: '29px' }} className="mt-6 text-center text-lg font-medium text-gray-800 leading-8">Reset Password</h2>
            <p className="mt-2 text-center leading-6 text-base text-gray-600">Please choose your new password.</p>
          </div>
          <div className="px-4 sm:px-10 mt-6">
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="flex flex-col gap-2">
                <label className="text-sm font-medium">New Password</label>
                <input type="password" placeholder="••••••••••••••" className="rounded-xl border border-gray-300 px-3 py-2" value={formData.password} onChange={(e) => setFormData({ ...formData, password: e.target.value })} />
                {errors.password && <span className="text-xs text-red-500">{errors.password}</span>}
              </div>
              <div className="flex flex-col gap-1.5">
                <div className="flex flex-col gap-2">
                  <label className="text-sm font-medium">Re-enter Password</label>
                  <input type="password" placeholder="••••••••••••••" className="rounded-xl border border-gray-300 px-3 py-2" value={formData.confirmpassword} onChange={(e) => setFormData({ ...formData, confirmpassword: e.target.value })} />
              ...
              <div className="mt-8">
                <button type="submit" disabled={resetting || !formData.password || !formData.confirmpassword || passwordsMismatch} className="w-full bg-blue-600 text-white py-3 px-4 rounded-lg hover:bg-blue-700 disabled:opacity-50">
                  {resetting ? <span className="flex items-center justify-center gap-2 py-1"><div className="animate-spin rounded-full border-2 border-blue-300 border-t-white w-5 h-5" /><span className="ml-2">Resetting</span></span> : <span className="flex items-center justify-center gap-2 py-1"><span className="ml-2">Reset Password</span></span>}
                </button>
              </div>
            </form>
          </div>
