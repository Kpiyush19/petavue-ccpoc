...
export default function SkillDetailPage() {
  ...
  if (!skill) {
    return (
      <div className="flex flex-col items-center justify-center h-full w-full gap-4 bg-grey-50">
        <p className="text-[16px] text-[var(--text-secondary)]">Skill not found.</p>
      ...
      <div className="flex flex-col w-full h-full min-w-[900px]">
        ...
        <div className="flex w-full px-6 items-center justify-between gap-4 h-[60px] shrink-0 border-b border-[var(--color-grey-100)] bg-white">
              <div className="flex items-center gap-2 min-w-0">
                ...
                <div className="relative min-w-0" ref={switcherRef}>
                  ...
                  >
                    <span className="block truncate text-[16px] leading-[24px] font-medium text-grey-900">{skill.name}</span>
                  ...
                  {switcherOpen && (
                    <div
                      role="listbox"
                      className="absolute top-full left-0 mt-1 bg-white flex flex-col rounded-lg border border-grey-200 z-50 min-w-[300px] max-w-[350px] overflow-hidden"
                    >
                      <div className="flex items-center gap-2 h-10 px-3 border-b border-grey-100 shrink-0">
                        ...
                        <input
                          ...
                          className="flex-1 min-w-0 text-[14px] bg-transparent border-none outline-none text-[var(--text-primary)] placeholder:text-[#adb2ce]"
                        />
                      </div>
                      <div className="max-h-[320px] overflow-y-auto py-1">
                        {switcherList.length === 0 ? (
                          <p className="px-3 py-4 text-[14px] text-[var(--text-muted)] text-center">No skills match.</p>
                        ) : (
                          switcherList.map((s) => {
                            ...
                            return (
                              <button
                                ...
                                className={cn(
                                  "flex items-center gap-2 w-full px-3 py-2 text-left bg-transparent border-none cursor-pointer transition-colors hover:bg-primary-50",
                                  active && "bg-primary-50/60"
                                )}
                              >
                                <span className={cn("flex-1 min-w-0 truncate text-[14px]", active ? "text-primary-700 font-medium" : "text-[var(--text-primary)]")}>{s.name}</span>
        ...
        <div className="flex-1 min-h-0 overflow-y-auto bg-grey-50 p-4">
          <div className="flex flex-col min-h-full w-full bg-white border border-grey-100/50 rounded-xl p-4">
              ...
              {outputImg && (
                <motion.div {...fadeUp(0.06)} className="flex flex-col gap-2">
                  ...
                  >
                    ...
                    <span className="absolute bottom-3 left-3 inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-grey-900/80 text-[12px] font-semibold uppercase tracking-wider text-white backdrop-blur-sm">
                      <Info size={12} /> Sample output
                    </span>
                    <div className="absolute inset-0 flex items-center justify-center bg-black/0 group-hover:bg-black/25 transition-colors">
                      <span className="opacity-0 group-hover:opacity-100 transition-opacity inline-flex items-center gap-1.5 px-3.5 py-2 rounded-full bg-white/95 text-[14px] font-medium text-[var(--text-primary)] shadow-lg">
                  ...
                  <div className="flex flex-col divide-y divide-grey-100 border border-grey-100 rounded-lg overflow-hidden mt-1">
                    {skill.inputs.map((inp, i) => (
                      <div key={i} className="flex items-start gap-3 px-4 py-3 bg-white">
                        <span className="text-[12px] font-semibold text-primary-500 tabular-nums shrink-0 mt-0.5">
            ...
            <motion.aside {...fadeUp(0.06)} className="w-[300px] shrink-0 self-start sticky top-0 flex flex-col gap-3 p-3 bg-grey-50 border border-grey-100/70 rounded-xl">
                {/* Readiness verdict — the first thing an operator needs: can I
                    run this right now? Sits directly under the Activate CTA. */}
                {totalIntegrations > 0 && (
                  isReady ? (
                    <div className="flex items-start gap-2.5 p-2 rounded-lg bg-[var(--color-green-bg)] border border-[var(--color-green)]/25">
                    ...
                    <div className="flex items-start gap-2.5 p-2 rounded-lg bg-amber-50 border border-amber-300">
                      ...
                      <div className="min-w-0">
                        ...
                        <button
                          ...
                          className="mt-2 text-[12px] font-medium text-primary-600 hover:text-primary-700 hover:underline bg-transparent border-none p-0 cursor-pointer"
                        >
                          Connect sources →
                        </button>
                      </div>
                ...
                <div className="flex flex-col gap-2.5">
                  ...
                  <ol className="flex flex-col gap-2">
                    {[
                      ...
                      <li key={i} className="flex items-start gap-2.5 text-[14px] leading-snug text-[var(--text-secondary)]">
                        <span className="flex items-center justify-center w-5 h-5 rounded-full bg-white border border-[var(--border-primary)] text-[12px] font-semibold text-primary-600 shrink-0">{i + 1}</span>
                            ...
                            return (
                              <div key={it.name} className="flex items-center gap-2.5" title={it.desc}>
                                {url ? (
                                  ...
                                  <Plug size={16} className={cn("shrink-0", connected ? "text-primary-500" : "text-[var(--text-muted)]")} />
                                )}
                                <span className={cn("flex-1 min-w-0 text-[12px] truncate", connected ? "text-[var(--text-primary)]" : "text-[var(--text-muted)]")}>{it.name}</span>
                                {connected ? (
                                  <span className="inline-flex items-center gap-1 text-[12px] font-medium text-[var(--color-green)] shrink-0">
                                  ...
                                  <button
                                    type="button"
                                    onClick={() => navigate("/settings")}
                                    className="text-[12px] font-medium text-primary-600 hover:text-primary-700 hover:underline shrink-0 bg-transparent border-none p-0 cursor-pointer"
                                  >
                                    Connect
                                  </button>
                                )}
