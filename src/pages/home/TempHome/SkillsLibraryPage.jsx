    ...
    <div className="flex items-center gap-1.5 min-w-0">
      {(tags || []).slice(0, 5).map((name) => {
        ...
        return url ? (
          ...
          <span key={name} title={name} className="px-2 py-0.5 text-[11px] rounded border bg-white text-[var(--text-secondary)] border-[var(--border-primary)] whitespace-nowrap">{name}</span>
        );
      })}
      {tags?.length > 5 && <span className="text-[11px] text-[var(--text-muted)]" title={tags.slice(5).join(", ")}>+{tags.length - 5}</span>}
    </div>
  );
}
          ...
          <div className="flex flex-col bg-white rounded-xl h-full w-full overflow-hidden border border-pv-neutral-grey-100">
            ...
            <div className="flex items-center justify-between gap-3 h-14 shrink-0 w-full border-b border-[var(--pv-neutral-grey-150)] px-4">
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div className="flex items-center gap-1 h-8 p-0.5 bg-pv-neutral-grey-100 rounded-lg shrink-0">
                ...
                <div className="flex items-center gap-0.5 p-0.5 bg-pv-neutral-grey-100 rounded-lg shrink-0">
                  {[{ id: "grid", Icon: LayoutGrid }, { id: "table", Icon: List }].map(({ id, Icon }) => (
                    <button key={id} onClick={() => setView(id)} aria-label={`${id} view`}
                      className={cn("flex items-center justify-center w-7 h-7 rounded-md border-none cursor-pointer transition-colors",
                        view === id ? "bg-white text-pv-primary-primary-600 shadow-sm" : "bg-transparent text-[var(--text-muted)] hover:text-[var(--text-primary)]")}>
                      <Icon size={15} />
                    </button>
                  ))}
                </div>
                  ...
                  {filtered.map((skill) => {
                    ...
                    return (
                      <div key={skill.id} onClick={() => open(skill.id)}
                        ...
                        <ArrowRight size={15} className="text-[var(--text-muted)] group-hover:text-pv-primary-primary-500 group-hover:translate-x-0.5 transition-all justify-self-end" />
                      </div>
                    );
                  })}
                </div>
