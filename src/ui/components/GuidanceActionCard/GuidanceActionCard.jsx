    ...
    >
      ...
      <div className="guidance-action-card__icon-frame">
        <div className="guidance-action-card__icon-container">
          <ListMagnifyingGlass size={36} weight="regular" color="var(--color-grey-600)" />
        </div>
      </div>
      ...
      <div className="guidance-action-card__caret">
        <CaretRight size={16} weight="regular" color="var(--color-grey-600)" />
      </div>
    </button>
  );
}
...
function StatusIcon({ status }) {
  ...
  if (status === 'done') {
    return <CheckCircle size={12} weight="fill" color="var(--color-green)" />;
  }
  return null;
}
