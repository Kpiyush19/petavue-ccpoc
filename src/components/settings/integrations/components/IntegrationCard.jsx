...
const IntegrationCard = ({
  ...
  return (
    <div className="flex flex-col gap-4 bg-white items-start justify-between border border-[var(--color-grey-200)] h-full w-full rounded-lg px-6 py-4">
      <div className="flex flex-col gap-4 w-full">
        <div className="flex justify-between">
          <div className="flex items-center gap-3">
            <img src={cardData.logo} alt="" width={24} />
            <p className="text-[var(--color-text-primary)]">{cardData.name}</p>
          </div>
          {cardData.connected && (
        ...
        <p className="font-normal text-xs leading-5 text-[var(--color-grey-500)]">{cardData.description}</p>
      </div>
      <div className="flex w-full items-center justify-between gap-5">
        {cardData.coming_soon && !cardData.connected ? (
          <div className="text-xs text-[var(--color-primary-500)] px-3 py-1.5 font-normal leading-5 rounded-lg bg-[var(--color-primary-50)]">
            Coming Soon
          </div>
        ) : !cardData.connected ? (
          isAuthIntegration ? (
          ...
          <Button variant="secondary" size="lg" onClick={handleViewDetails}>
            View Details
          </Button>
        )}
        {/* {syncStatus?.hasActiveSync && syncStatus?.currentSession?.status !== "COMPLETED" && (
          <span className="flex items-center gap-1.5 px-2 py-1 bg-[var(--color-primary-50)] rounded-lg">
            <CloudArrowUp size={10} className="shrink-0 text-[var(--color-primary-500)]" />
            <span className="whitespace-nowrap text-xs text-[var(--color-primary-500)]">
        ...
        {(syncStatus?.currentSession?.status === "COMPLETED" ||
          ...
          <span className="flex items-center gap-1.5 px-2 py-1 bg-[var(--color-green-bg)] rounded-lg">
            <CloudArrowUp size={10} className="shrink-0 text-[var(--color-green)]" />
            <span className="whitespace-nowrap text-xs text-[var(--color-green)]">
              {syncStatus?.currentSession?.displayStatus || "Synced"}: 100%
            </span>
          </span>
        )}
        {cardData.connected && !syncStatus?.hasActiveSync && !syncStatus?.lastCompletedAt && (
          <span className="flex items-center gap-1.5 px-2 py-1 bg-[var(--color-grey-50)] rounded-lg">
            <CloudArrowUp size={10} className="shrink-0 text-[var(--color-grey-400)]" />
            <span className="whitespace-nowrap text-xs text-[var(--color-grey-500)]">Never synced</span>
          </span>
        )}
