  ...
  return (
    <div className={twMerge("flex flex-col gap-1.5", !dynamicWidth && "w-full", twJoin(className?.wrapper && className.wrapper))}>
      {label ? (
        <label
          htmlFor={id}
          className={twMerge("text-brand-ai-1100 text-sm", twJoin(className?.label && className.label))}
        >
          {label}
          {required ? <span className="text-brand-ai-100">*</span> : null}
        </label>
      ) : null}
      <div
        ref={divRef}
        className={twMerge(
          "flex flex-1 items-center border border-pv-neutral-grey-200 rounded-lg py-3 px-4 bg-white focus-within:border-brand-ai-100",
          twJoin(
            focusInput && "border-brand-ai-100",
            !disabled && !focusInput && "hover:border-pv-primary-primary-300",
            disabled && "bg-pv-neutral-grey-50 cursor-not-allowed",
      ...
      >
        ...
        {type !== "textarea" ? (
          <PRInputText
            ...
            pt={{
              root: {
                className: twMerge(
                  "pv-input shadow-none rounded-none w-full resize-none outline-none text-sm text-brand-ai-1100 placeholder:text-pv-text-secondary-text bg-transparent autofill:[-webkit-text-fill-color:inherit] [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none",
                  twJoin(className?.input?.root && className.input.root),
                  disabled
                    ? "cursor-not-allowed autofill:shadow-[inset_0_0_0px_1000px_var(--pv-neutral-grey-50)]"
          ...
          <PRInputTextarea
            ...
            pt={{
              root: {
                className: twMerge(
                  "pv-textarea shadow-none rounded-none w-full resize-none outline-none text-sm text-brand-ai-1100 placeholder:text-pv-text-secondary-text bg-transparent",
                  twJoin(className?.input?.root && className.input.root),
        ...
        {showClearInput && value?.length > 0 && (
          <XCircle
            size={16}
            className={twMerge(
              "text-pv-neutral-grey-600 hover:text-pv-neutral-grey-900 ml-2 cursor-pointer",
        ...
        {isPassword && (
          <Button
            ...
            mainBtnClassName="ml-2 px-1 py-0"
          >
