...
3. **How do others (like Fibbler) solve it?**
...
1. **Read in order.** Modules build on each other, 1 → 11 (plus the pain-landscape appendix). Don't skip to Fibbler (M11) before the foundation.
...
| 11 | Pain ↔ use case (Fibbler) | The keystone: pain vs use case vs feature vs format — all 7 Fibbler use cases mapped, and the pains it does NOT solve. |
| — | The real pain landscape (appendix) | The prioritized field-pain map: what marketers actually complain about, who solves it, where Petavue fits. |
...
- **Layer B · The data/ops layer (why it's hard)** — The plumbing that blocks the goal: missing data, broken integrations, anonymity, messy CRM, slow process. This is where the *difficulty* lives, and where tools like Fibbler operate.
...
Meet **Maya**, demand-gen lead at **Lumen** (B2B SaaS, ~$20k deals, ~3-month cycle). She's the person Fibbler sells to, so her head is the most important one to live inside. Her one-line goal: **create efficient, provable pipeline.** She's judged on pipeline created/influenced and on efficiency (cost per pipeline dollar).
...
> **From the field —** This is the exact gap Adam (Fibbler) lived as a marketer, and why his pitch lands: he's not selling "a dashboard," he's selling Maya the ability to walk into the QBR and *defend her budget with proof*. Same data, but aimed at her Layer-A fear.
...
- **Layer B pain:** tools that don't talk to each other; this *is* their daily life. Often the one who installs Fibbler.
...
- **Layer B pain:** data breaks exactly at the marketing↔sales handoff — the seam Fibbler stitches.
...
> **From the field — the vanity-metric trap.** Impressions, CTR and CPC are easy to inflate and feel good, but on their own they are **vanity metrics** — they say nothing about revenue. The marketers worth learning from (Adam included) judge ads by the *pipeline* they produce, not the clicks. Always push a top-funnel number down to a bottom-funnel outcome before trusting it.
...
> **⚓ Loop back to the anchor —** The customer's real pain here isn't "I want a channel report" (a format). It's **"I'm afraid I'm wasting half my budget and can't prove where to shift it."** Fibbler answers it for one channel (LinkedIn). The unmet, higher-value version is answering it **across all channels at once** — which needs the flexible, cross-source engine that is Petavue's opening.
...
> **From the field — Why Fibbler's trick works anyway.** Fibbler builds a join key at the **company** level — LinkedIn exposes *which companies* engaged, and the CRM has companies as accounts. So it links "Acme engaged" → "Acme became a deal" **without ever needing the person's name**. That's how it beats the anonymity — though it's still blind to dark-funnel and multi-touch nuance. (M8 dissects this whole data layer.)
...
- **Influence-based / incrementality** — A different question entirely. Not "who gets the slice?" but **"did being exposed make a deal more likely at all?"** — measured by comparing exposed vs unexposed groups (lift). This is what B2B (and Fibbler's Lift Analysis) leans on.
...
These aren't metrics — they're the hard-won mental models experienced operators carry. The kind Adam teaches on YouTube. Knowing these is what makes you sound like a practitioner, not a student.
...
| --- | --- | --- | --- |
| **Anonymity** | Ad/site engagement isn't tied to a named person until they identify themselves. | Data | **Partly** — resolve at company level (Fibbler's trick) |
...
### What Fibbler's benchmarks page is

Fibbler's "Benchmarks" use case is the **external · industry** type: it shows how your LinkedIn-ad numbers (CTR, CPL, engagement) compare to peers, so a marketer can answer "is my 0.4% good?" It's a **descriptive, context-giving** use case (M7) — useful, low-risk, and the kind of thing that needs scale data Fibbler has from many customers.
...
> **⚓ Loop back to the anchor —** When a customer says "I can't see my customer journeys," *this* is the mess they mean — and the wrinkle column is *why* it hurts. Fibbler's "Customer Journeys" use case tries to make a slice of this visible (the LinkedIn touches). Reading these 20, you can now judge precisely **which scenarios Fibbler illuminates and which it leaves dark** — the exact lens you need for the teardown.

---

## Module 11 — Pain ↔ Use case: connecting the dots (the keystone)

*The single most-confused pair in this whole world. Get these two entities and their relationship clean, and you'll never again let "dashboard," "use case," "feature," and "pain" get used interchangeably. Worked end-to-end on Fibbler's 7 use cases.*

### Four terms that get used interchangeably

- **Pain** — A problem in the **user's own words**, stated as an **outcome they care about**, with no product named in it.
  - *Sounds like:* "I can't prove my LinkedIn ads create pipeline, so my budget's always under threat."
- **Use case** — The **named, repeatable analysis** a product runs to answer a pain. Product/analyst language. Often **named after the pain** it kills.
  - *Sounds like:* "Revenue attribution." "Lift analysis." A noun-phrase you'd put on a pricing page.
- **Mechanism / feature** — The **plumbing** that makes a use case possible. Useful, but never the reason someone buys.
  - *Sounds like:* "Sending data back to the CRM" (CRM Sync). This is plumbing — **not** a pain.
- **Delivery format** — How the answer is **served & acted on**: dashboard, alert, report, recommendation. Downstream of the pain.
  - *Sounds like:* "A dashboard." "A Slack alert." The means of delivery — not the problem itself.

> **Key — The test to tell them apart.** If the sentence names a product thing (dashboard, sync, report) → it is a **format / mechanism**. If it is a noun phrase you would put on a pricing page → **use case**. If it is an outcome-level problem with no product in it → **pain**. Only the last one is what the customer is actually buying.

> **Key — The altitude of a pain.** State a pain at the right level. Too low and it is only one **instance** ("I can't measure my *Meta* ads' impact") — the channel is a swappable variable, so it is really one case of a broader pain. Too high and it is a platitude ("I want to grow"). The buildable altitude is where the specific variable is abstracted but the problem stays concrete: "I can't measure which of my spend actually drove revenue." **Test:** if you can swap a noun (Meta → Google → LinkedIn) and it is still the same complaint, lift it a level until that noun becomes a parameter, not the pain. Altitude also decides the build form: an instance → a **feature**; the abstracted pain → a **use case**; the overarching job → the **product**.

### The chain that connects every dot

A pain doesn't float alone. It sits in a chain — and a product plugs in at the use-case link to relieve it. Read left to right:

`Persona (Maya, demand gen)` → `Goal (provable pipeline)` → `Pain ("can't prove ads→revenue")` → `Use case (Revenue Attribution)` → `Method (influence attribution)` → `Metric (influenced pipeline $)` → `Format (dashboard / report)` → `Action (defend & reallocate budget)`

…all landing at a point on the **customer journey** (here: top-funnel engagement linked to bottom-funnel revenue).

> **Key — The relationship, in one line.** A **use case is an instrument aimed at a pain.** And one **core pain spawns a whole suite of use cases** (each a different facet). So a **product = a bundle of use cases pointed at one core pain.**

### Fibbler, decoded: one core pain, seven facets

> **⚠ The pain — Fibbler's core pain.** **"LinkedIn ad spend is a black box between my money and my revenue."** Every one of the 7 use cases attacks a different facet of that one black box.

| Use case | The pain it kills (user's words) | What it computes | Funnel stage | Action it unlocks |
| --- | --- | --- | --- | --- |
| **Revenue Attribution** | "I can't prove my ads drove pipeline/revenue." | Links ad engagement → CRM pipeline & revenue (influence-based). | [Top]→[Bottom] | Defend & justify spend. |
| **Lift Analysis** | "Maybe those deals would've closed anyway — did ads *cause* it?" | Incrementality: exposed vs unexposed comparison. | Across | Prove causation, not just correlation. |
| **Customer Journeys** | "I can't see the path an account took; the middle is dark." | Reconstructs every ad touch per account over time. | [Top]→[Bottom] | See what sequence works; tell the story. |
| **Intent Signals** | "I don't know which accounts are warming *now*; SDRs waste time." | Scores account engagement intensity, near real-time. | [Mid] | SDR prioritizes hot accounts (alert→action). |
| **Campaign Optimization** | "Which campaigns/creatives drive *quality* pipeline, not just clicks?" | Campaign/creative performance tied to downstream pipeline. | [Top] (via [Bottom]) | Kill/scale campaigns; shift budget. |
| **Benchmarks** | "Is my CTR/CPL good or bad? I've no reference." | Compares your metrics vs peers/industry. | [Top] | Set expectations; spot under/over-performance. |
| **CRM Sync** *(mechanism)* | (Not a pain.) "The engagement data lives in LinkedIn, not where sales works." | Writes ad engagement back into the CRM. | [Bottom] | **Enables the other six** — it's the plumbing. |

> **From the field — Why this matters for your reading.** When you open Fibbler's 7 pages, don't read them as 7 features. Read each as: *which facet of the black-box pain does this attack, and at which journey point?* That's the difference between listing what a competitor has and understanding why anyone pays for it.

### The pains Fibbler does NOT cover (Petavue's opening)

Fibbler owns one black box: **LinkedIn/Google ad spend → revenue.** But the marketer has bigger pains around it that a narrow tool can't touch. These are where a flexible engine could go:

| Bigger pain (user's words) | Use case it needs | Does Fibbler do it? |
| --- | --- | --- |
| "Across *all* channels — Google, Meta, LinkedIn, events, organic — where's my next dollar best spent?" | Cross-channel / full-funnel attribution & media mix | Partial (LinkedIn + Google only). |
| "Which leads/accounts are even worth my team's time?" | Lead / account scoring (ICP fit) | No. |
| "Why did pipeline (or CPL) move this quarter? What changed?" | Diagnostic / root-cause analysis | No. |
| "What's my true CAC and LTV by channel? Is this efficient?" | Unit-economics analysis | No. |
| "Will we hit the number, and where's the gap?" | Pipeline coverage & forecasting | No. |

> **Key — Go deeper.** This is just the Fibbler-adjacent slice. For the full prioritized map of what marketers actually complain about — across all channels, with who-solves-what and where Petavue is strongest — see **The Real Pain Landscape** (appendix below).

> **⚓ Loop back to the anchor —** Fibbler nails **one core pain** with a tight **suite of use cases**, delivered simply. The marketer's *other* top pains (cross-channel spend, lead quality, diagnostics, unit economics) sit right next door, mostly unsolved by Fibbler. That adjacency is the whole strategic question for Petavue: **do we win by going deeper on Fibbler's box, or broader across the pains it can't reach?** But now you have the vocabulary to argue it precisely instead of mixing up pains, use cases, features, and formats.

---

## Appendix — The real pain landscape (companion to M11)

*Worked examples that reinforce the pain vs use case distinction, then the broader map: the pains marketers actually raise in the field, who solves them (or does not), and where Petavue can step in. Grounded in 2025 community and survey data.*

### Worked examples: pain vs use case vs format

The same structure each time: the **pain** contains no product, the **use case** is the named analysis, the **format** is how it is served. None of these are Fibbler. Note the altitude — each pain is stated so the specific channel or tool is a swappable variable, not the pain itself.

| Pain (no product in it) | Use case | Format (not the pain) |
| --- | --- | --- |
| **Paid acquisition:** "I'm investing across paid channels but can't tell which spend actually drove incremental revenue, versus revenue I would have earned anyway." *(the channel — Meta, Google, LinkedIn — is the variable)* | Incrementality analysis | A lift / ROAS dashboard — the means of delivery. The pain is the wasted spend and the doubt. |
| **Retention:** "I learn a customer is leaving only after they have effectively decided to go." | Churn-risk / account-health scoring | An alert. The pain is the revenue lost to a late warning. |
| **Sales leadership:** "I can't trust the forecast — deals I'm told will close keep slipping." | Forecasting / pipeline-coverage analysis | A forecast report. The pain is missing the number and the fallout. |

> **Key — The principle.** One pain maps to one or a few use cases, each delivered through a format. Vendors lead with the format ("dashboard," "alert," "report"); what the customer buys is relief from the pain. Always reverse-engineer from the pain — stated at the right altitude.

### The field-pain map (priority-ordered)

What B2B marketers, demand-gen and RevOps people actually raise in communities and surveys. Each row gives a **plain-language definition** of the pain, the quote it came from, its **underlying cause**, and where Petavue fits. Ordered by priority = frequency × data-layer difficulty. ★ marks where Petavue is strongest.

> **Example — The four underlying causes.** **Data gap** = the data needed is missing, anonymous, or siloed. **Process gap** = the data could exist, but workflow / CRM hygiene breaks it. **Mechanism gap** = the connection or plumbing to link or compute it is not built. **Analysis gap** = the data exists, but no one runs the analysis that answers it. The cause decides the fix — and whether a product like Petavue can address it at all.

**1. [High · Full] Cross-channel spend efficiency** — Can't compare each channel's true contribution to revenue, so budget can't move to the best-returning channels.
- *Quote:* "Across all channels — Google, Meta, LinkedIn, events, organic — where's my next dollar best spent?" · only 18% use integrated cross-channel attribution
- **Cause:** Data gap — each ad platform is a walled garden that self-reports; no unified cross-channel view of revenue.
- **Use case needed:** Cross-channel / media-mix attribution
- **Solved today?** Partial — Dreamdata, HockeyStack (B2B). Single-channel tools (Fibbler) can't. MMM is heavy/enterprise.
- **Petavue angle:** ★ Strong fit — one flexible engine across all their data.

**2. [High · Full] Revenue attribution** — Can't credibly connect marketing activity to closed revenue, so marketing's value can't be demonstrated.
- *Quote:* "I can't prove marketing drove revenue, so I can't defend my budget." · 70% pressured to prove ROI; ~90% still on last-touch
- **Cause:** Data gap — engagement is anonymous and touches aren't tied to deals; most default to last-touch.
- **Use case needed:** Revenue attribution
- **Solved today?** Partial — many attribution tools, but trust is low; most default to last-touch.
- **Petavue angle:** Fit, but crowded — must win on depth/trust, not just existence.

**3. [High · Mid/Top] Lead/account quality** — High lead volume but poor fit or intent, so sales effort is spent on prospects unlikely to convert.
- *Quote:* "I generate plenty of leads, but they're low quality — sales wastes time while high-intent ones slip." · 61% say lead quality is their #1 challenge
- **Cause:** Data gap — incomplete CRM data makes fit/intent scoring unreliable; and no scoring is applied.
- **Use case needed:** Lead / account scoring (ICP fit), predictive
- **Solved today?** Partial — 6sense, Demandbase, predictive tools, but choke on poor CRM data.
- **Petavue angle:** Fit where their data is clean enough to score.

**4. [High · Bottom/Full] Forecast reliability** — Pipeline data can't be trusted, so revenue forecasts are inaccurate and the number gets missed.
- *Quote:* "Our pipeline reports and forecast aren't trustworthy; we keep missing the number." · data is backward-looking
- **Cause:** Process gap — CRM hygiene and late/optimistic rep entry corrupt the pipeline; data also lags.
- **Use case needed:** Pipeline coverage + forecasting + data-quality diagnostics
- **Solved today?** Partial — Clari etc. for forecasting; CRM-native is weak.
- **Petavue angle:** Fit — especially the "why is the data wrong" diagnostic.

**5. [High · Mid→Bottom] GTM alignment** — Sales and marketing work from different numbers, causing credit disputes and wasted effort.
- *Quote:* "Sales and marketing fight over who gets credit; we're looking at different numbers." · IDC: 10%+ revenue lost; 86% can't tie multiple stakeholders to opps
- **Cause:** Mechanism gap — data breaks at the marketing→sales handoff; no shared source of truth.
- **Use case needed:** Unified attribution + buying-group stitching
- **Solved today?** Partial — ABM platforms + RevOps glue; messy in practice.
- **Petavue angle:** Fit — a single source of truth across the handoff.

**6. [Med · Full] Diagnostic insight** — Can't explain why a metric moved, so teams can't act on the underlying cause.
- *Quote:* "Why did our numbers move this quarter — CPL up, pipeline down? What changed?"
- **Cause:** Analysis gap — the data largely exists; what's missing is root-cause analysis. Dashboards show what, not why.
- **Use case needed:** Diagnostic / root-cause analysis
- **Solved today?** Mostly no — dashboards show *what*, almost never *why*.
- **Petavue angle:** ★ Strongest fit — diagnostic depth is Petavue's core edge.

**7. [Med · Full] Account journey visibility** — Can't see the full set of people and touches within an account before purchase.
- *Quote:* "I can't see the whole buying committee's journey for an account." · 86% struggle to tie stakeholders to opps
- **Cause:** Data gap — identity stitching across multiple people and touches within an account is hard.
- **Use case needed:** Account / buying-group journey analytics
- **Solved today?** Partial — 6sense, Demandbase, powerful but pricey/complex.
- **Petavue angle:** Fit at account level.

**8. [Med · Top (via Bottom)] Campaign-to-pipeline optimization** — Can't tell which campaigns or creatives produce quality pipeline rather than cheap clicks.
- *Quote:* "Which campaigns/creatives drive quality pipeline, not just cheap clicks?"
- **Cause:** Mechanism gap — no closed loop connecting a campaign to the downstream pipeline it produced.
- **Use case needed:** Campaign optimization tied to pipeline
- **Solved today?** Partial — Fibbler (LinkedIn only); single-channel tools.
- **Petavue angle:** Fit cross-channel.

**9. [Med · Full/Post] Unit economics** — Can't measure true CAC and LTV by channel/segment to judge efficiency.
- *Quote:* "What's my true CAC and LTV by channel/segment — are we actually efficient?"
- **Cause:** Analysis gap — inputs are spread across ad platforms, CRM and finance; no unified computation.
- **Use case needed:** Unit-economics analysis
- **Solved today?** Partial — manual BI / finance spreadsheets.
- **Petavue angle:** Fit.

**10. [Med · Timing] Decision latency** — Insights arrive too late to influence the decision they were meant to inform.
- *Quote:* "By the time I see the data, the market's shifted and budget's already locked."
- **Cause:** Process gap — reporting cycles lag and pipelines are backward-looking.
- **Use case needed:** Faster / near-real-time analysis
- **Solved today?** Mostly no — reporting lag is the norm.
- **Petavue angle:** Fit — speed of analysis.

> **Example — How to read priority.** **High** = raised constantly in surveys/communities *and* hard because of a data-layer cause (anonymity, walled gardens, dirty CRM, identity stitching). **Med** = real and frequent, but either narrower or partly solvable with existing process. The two ★ rows — **cross-channel spend** and **"why did it change" diagnostics** — are where Petavue's flexible, deep-analysis engine is most differentiated versus narrow point tools.

> **⚓ The strategic read —** Notice the shape: the **High-priority pains are mostly "full-funnel" and cross-channel** — exactly the ones a *narrow* tool like Fibbler can't reach, and exactly where a flexible engine can. Fibbler chose to own one sharp box brilliantly. The open territory is the **connective, diagnostic, cross-channel** pains next door. Whether Petavue wins by going *deeper* into a box or *broader* across these is the strategic question — but now you can argue it from a real, prioritized pain map.

*Sources: State of B2B Marketing Attribution 2025 (RevSure); B2B demand-gen 2025 surveys (lead-quality 61%, ROI pressure 70%); IDC sales-marketing misalignment; cross-channel attribution adoption (~18%). Stats are directional, for prioritization — verify before quoting externally.*
